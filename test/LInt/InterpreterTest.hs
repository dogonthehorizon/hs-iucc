...
test_interpInt = testProperty
  ...
  $ do
    i <- forAll genInteger
    i' <- LInt.interpretExpr $ LInt.IntLit i
    i === i'
...
test_interpNeg = testProperty
  ...
  $ do
    i <- forAll genInteger
    i' <- LInt.interpretExpr $ LInt.IntNeg (LInt.IntLit i)
    (-i) === i'
...
test_interpPlus = testProperty
  ...
  $ do
    i <- forAll genInteger
    j <- forAll genInteger
    r <- LInt.interpretExpr $ LInt.IntPlus (LInt.IntLit i) (LInt.IntLit j)
    (i + j) === r
...
test_interpMinus = testProperty
  ...
  $ do
    i <- forAll genInteger
    j <- forAll genInteger
    r <- LInt.interpretExpr $ LInt.IntMinus (LInt.IntLit i) (LInt.IntLit j)
    (i - j) === r
...
test_interpRead = testProperty
  ...
  $ do
    i <- forAll genInteger
    r <- LInt.interpretExpr' (pure i) LInt.IntRead
    i === r
